...
export default function App(){
  ...
  const topic=library?.topics.find(t=>t.id===(route.params.get('topic')||snapshot?.settings.topicId))||library?.topics[0];
  ...
  async function mode(limit:number){try{await store.enqueue('settings',{dailyNewLimit:limit});setNotice(`已切换为每日 ${limit} 道新题，沿用今天已学数量。`);}catch(e){setNotice(errorText(e));}}
