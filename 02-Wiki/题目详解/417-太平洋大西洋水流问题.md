# 417. 太平洋大西洋水流问题 (Medium)

> **专题归类：** [[02-Wiki/专题总结/07-图论]]
> **LeetCode 链接：** https://leetcode.cn/problems/pacific-atlantic-water-flow/

## 一页复盘

- **题目总结：** 找出所有能够沿非增高度路径同时流到太平洋和大西洋的格子。
- **重要思考：** 从每个格子正向试流会重复搜索；分别从两片海岸反向走向高度不低的邻格，两个可达集合的交集就是答案。
- **必须考虑的情况：**
  - 反向移动条件是“邻格高度大于等于当前高度”，相等高度也可达。
  - 两片海岸各自有两条边界，角落和单行/单列会重复加入，需由 `visited` 去重。
  - 只有同时出现在两个可达集合中的格子才能输出。

## 题目目标

找出既能流向太平洋又能流向大西洋的格子。

## 重点

- 反向思考：从两片海岸向高处 DFS，而不是从每个格子向低处试流。
- 两次可达集合的交集即答案。

## 时间/空间复杂度

| 项目 | 复杂度 |
|---|---|
| 时间复杂度 | O(mn) |
| 空间复杂度 | O(mn) |

## 代码实现

```python
from typing import List


class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        rows, cols = len(heights), len(heights[0])
        pacific, atlantic = set(), set()
        def dfs(r, c, seen):
            seen.add((r, c))
            for dr, dc in ((1,0),(-1,0),(0,1),(0,-1)):
                nr, nc = r + dr, c + dc
                if 0 <= nr < rows and 0 <= nc < cols and (nr, nc) not in seen and heights[nr][nc] >= heights[r][c]:
                    dfs(nr, nc, seen)
        for r in range(rows):
            dfs(r, 0, pacific); dfs(r, cols - 1, atlantic)
        for c in range(cols):
            dfs(0, c, pacific); dfs(rows - 1, c, atlantic)
        return [[r, c] for r in range(rows) for c in range(cols) if (r, c) in pacific and (r, c) in atlantic]
```

## 易错点与复盘

- 反向 DFS 的高度条件是 `next >= current`。
- 两海的边界都要作为起点。

## 示例

```text
同时能反向到达两侧边界的格子才输出。
```
