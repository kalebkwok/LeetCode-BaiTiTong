...
- **题目总结：** 统计网格中由四方向相邻陆地组成的连通分量数量。
- **重要思考：** 每遇到一块未访问陆地，就发现了一座新岛；随后用 DFS/BFS 一次性标记整座岛，外层搜索的启动次数就是答案。
- **必须考虑的情况：**
  - 只允许上下左右连通，斜对角陆地不属于同一座岛。
  - 若使用“沉岛法”会修改输入；不允许修改时需另建 `visited`。
  - 全水、全陆和细长大岛都要正确；Python 递归 DFS 还要防止爆栈。
