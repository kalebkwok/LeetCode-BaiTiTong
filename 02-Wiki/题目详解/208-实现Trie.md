...
- **题目总结：** 用字符作为边构建前缀树，支持插入、完整单词查询和前缀查询。
- **重要思考：** 路径存在只代表前缀存在；必须额外保存单词结束标记，才能区分 `search("app")` 与 `startsWith("app")`。
- **必须考虑的情况：**
  - 多个单词可以共享前缀，插入时要复用已有节点。
  - 重复插入不能破坏结构，完整单词查询必须检查结束标记。
  - 遍历中任一字符不存在应立即失败；前缀存在不等于完整单词存在。
