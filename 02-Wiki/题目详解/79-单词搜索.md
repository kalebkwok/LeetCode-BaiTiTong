...
- **题目总结：** 判断能否在字符网格中通过四向相邻格子依次拼出目标单词。
- **重要思考：** 搜索状态是“当前位置 + 已匹配下标”；`visited` 只限制当前路径，因此选择格子后标记、回溯返回时恢复。
- **必须考虑的情况：**
  - 同一格子不能在同一条路径中重复使用，但可以被另一条起始路径重新使用。
  - 越界、字符不匹配或已经在当前路径中使用时立即剪枝。
  - 找到完整单词后立即返回；失败分支必须恢复现场，不能使用永久全局访问标记。
