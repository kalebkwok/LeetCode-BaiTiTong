...
- **题目总结：** 判断课程先修关系构成的有向图是否无环，也就是能否完成全部课程。
- **重要思考：** 对关系 `[course, pre]` 建边 `pre → course`；Kahn 算法不断移除入度为 `0` 的点，最终处理数等于课程总数才说明无环。
- **必须考虑的情况：**
  - 没有先修关系、孤立课程也要加入图并能被统计。
  - 自环或任意有向环都会导致无法完成。
  - 若输入可能包含重复边，需要去重，或保证入度与邻接表对重复边的处理一致。
